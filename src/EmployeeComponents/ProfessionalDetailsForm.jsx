...
const ProfessionalDetailsForm = ({ formData, onNext, onBack, onFormDataChange }) => {
    ...
    useEffect(() => {
        const fetchEmployees = async () => {
            ...
            try {
                const response = await fetch('https://backend-aa.azurewebsites.net/api/v1/employeeManager/employees', {
                    method: 'GET',
                    headers: {
    ...
    const handleSubmit = async (event) => {
        ...
        if (Object.keys(validationErrors).length === 0) {
            try {
                ...
                const response = await fetch(`https://backend-aa.azurewebsites.net/api/v1/employeeManager/exists/${formData.employeeId}`, {
                    method: 'GET',
                    headers: {
